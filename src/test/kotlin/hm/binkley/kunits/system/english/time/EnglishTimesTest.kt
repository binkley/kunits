...
internal class EnglishTimesTest {
    ...
    fun `should pretty print`() {
        "$Atom" shouldBe "English atom"
        "${1.atoms}" shouldBe "1 atoms"
        "$Second" shouldBe "English second"
        "${1.seconds}" shouldBe "1 sec"
        "$Minute" shouldBe "English minute"
        "${1.minutes}" shouldBe "1 min"
        "$Moment" shouldBe "English moment"
        ...
        "$Quadrant" shouldBe "English quadrant"
        "${1.quadrants}" shouldBe "1 quadrants"
        "$Day" shouldBe "English day"
        "${1.days}" shouldBe "1 days"
        "$Week" shouldBe "English week"
        "${1.weeks}" shouldBe "1 weeks"
        "$Fortnight" shouldBe "English fortnight"
        "${1.fortnights}" shouldBe "1 fortnights"
        "$Quinzième" shouldBe "English quinzième"
